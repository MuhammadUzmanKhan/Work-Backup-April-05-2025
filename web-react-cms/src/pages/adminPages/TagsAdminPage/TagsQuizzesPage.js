...
import { deleteTagsByID, getQizzesTags, getQuizByID } from 'store/actions/tag'
...
function TagsQuizzesPage() {
  ...
  const checkLink = async (id = tagId, reload = false) => {
    setLoading(true)
    const data = await dispatch(getQuizByID(`${id}/entity_links`))
    if (data.payload) {
  ...
  const goToFunction = async (id, type) => {
    setLoading(true)
    if (type === LINKED_ENTITIES_TYPES.QUIZ_THEME) {
      anchorRef.current.href = `#/quiz-theme/${id}`
    } else if (type === LINKED_ENTITIES_TYPES.QUIZ) {
  ...
  return (
    <>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h3">{t('admin.quizTags')}</Typography>
        <Permissions permission={'write:tag'}>
          <Link component={Route} to="/tags/quizzes/0" color="#000000" underline="none">
            <BaseButton
...
export default TagsQuizzesPage
