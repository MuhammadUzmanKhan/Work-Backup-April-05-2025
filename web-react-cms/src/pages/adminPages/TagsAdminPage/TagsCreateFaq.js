...
import { editTags, getTagsByID, postTags } from 'store/actions/tag'
...
function TagsCreateFaq() {
  ...
  useEffect(async () => {
    if (+id !== 0) {
      const data = await dispatch(getTagsByID(id))
  ...
  const cb = (res) => {
    ...
    if (+id === 0) navigate(`/tags/faq/${res.id}`, { replace: true })
  }
  const handleSubmit = (values) => {
    if (+id === 0) {
      dispatch(postTags({ params: { name: values.name }, cb }))
    } else {
      let obj = {
    ...
    <>
      <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between' }}>
        <Typography variant="h3">
          {+id !== 0 ? t('admin.editFaqTag') : t('admin.addFaqTag')}
        </Typography>
        {+id !== 0 && (
          <ToggleButton
            sx={{
