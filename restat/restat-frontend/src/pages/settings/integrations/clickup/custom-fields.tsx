...
import { CustomFieldsAvailable, apis } from '../../../../services';
...
const staticFields: ClickupCustomField[] = [
  {
    id: 'name',
    name: 'Title',
    type: 'text',
...
const CustomFields: React.FC<CustomFieldsProps> = ({ on, off, listId, onContinue, loading }) => {
  const [load, setLoad] = useState<boolean>(false);
  const [customFields, setCustomFields] = useState<ClickupCustomField[]>(staticFields);
  ...
  const [selectedFields, setSelectedFields] = useState<SelectedFields[]>(
    customFields.map(field => ({
      ...
      value: field.required ? CustomFieldsAvailable[0].id : '',
      customFieldName: field.required ? (field.isStaticField ? field.name : CustomFieldsAvailable[0].name) : '',
    }))
  );
  ...
  const getFields = async () => {
    try {
      ...
      setMembers(members);
      setCustomFields([...staticFields, ...custom_fields]);
      setLoad(false);
    } catch (error: any) {
    ...
    if (integratedCustomFields.current?.length) {
      ...
      setSelectedFields(
        customFields.map(field => ({
          ...
          isBlank: field.required && field.id ? false : true,
          value: field.required ? CustomFieldsAvailable[0].id : '',
          name: field.required ? CustomFieldsAvailable[0]?.name : '',
        }))
      );
    }
    off();
                  ...
                  <div key={index} className='flex justify-between p-2'>
                    ...
                    <Select
                      ...
                      onChange={value => handleFieldSelect(
                        ...
                        value,
                        field.isStaticField,
                        CustomFieldsAvailable.find(x => x.id === value)?.name,
                        field.type
                      )}
                      options={
                        members.map(member => ({ label: `${member.username} | ${member.email}`, value: '' + member.id }))
                      }
                      defaultValue={selectedFields.find(sf => field.id === sf.key)?.name || (field.required ? CustomFieldsAvailable[0].id : undefined)}
