...
const LinkedinConnectByStatesChart = React.memo(({ loading }: { loading: boolean }) => {
  const connectionsCountByState = useSelector((state: RootState) => state?.linkedinCounts?.connectionsCountByState);
  ...
  useLayoutEffect(() => {
    ...
    polygonTemplate.tooltipText = `
      ...
      Prospects Count: {prospectsCount}
    `;
    ...
    const countryMap = connectionsCountByState.map(item => ({
      ...
      state: item.state,
      value: parseInt(item.connectionsCount),
      connectionsCount: parseInt(item.connectionsCount),
      prospectsCount: parseInt(item.prospectsCount),
    }));
...
export default LinkedinConnectByStatesChart;
