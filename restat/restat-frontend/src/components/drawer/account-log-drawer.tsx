...
import { apis, ACCOUNT_LOG_TYPE, AccountLogType } from "../../services";
import { convertDateFormat } from "../../services/utils/convertDate";
import { customNotification } from "..";

interface accountLogData {
  date: string;
  time: string;
  user: { name: string };
  message: string;
  dealLogType: ACCOUNT_LOG_TYPE;
  updatedAt: string;
}

const AccountLogDrawer = ({ onClose, open, id }: { onClose: () => void, open: boolean, id: string }) => {
  const [logsData, setLogsData] = useState<accountLogData[]>([])
  const [filteredData, setFilteredData] = useState<accountLogData[]>([])
  ...
  const fetchLeadsLog = async (bidId: string = "", accountId: string) => {
    try {
      ...
      setLogsData(response?.data?.accountLogs)
      setFilteredData(response?.data?.accountLogs)
    }
    catch (error: any) {
      if (error.response?.data?.message) {
        customNotification.error("Error!", error.response?.data?.message || "Error in fetching account logs")
      }
    } finally {
      setLoading(false)
    }
          ...
          {filteredData.map((item, index) => (
            <Timeline.Item key={index}>
              <Typography style={{ whiteSpace: 'pre-line' }}>
                {item?.dealLogType === ACCOUNT_LOG_TYPE.ACCOUNT_UPDATED ? (
              ...
              <Typography.Text keyboard style={{ color: "#1A4895" }}>{item?.user?.name}</Typography.Text>
...
export default AccountLogDrawer