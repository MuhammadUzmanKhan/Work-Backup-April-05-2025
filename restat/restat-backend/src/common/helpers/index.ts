...
import { IMarketplaceJobPosting, IMessage } from "src/types/bids";
...
export const createJobUrlFromJobId = (jobId: string) => {
  return `https://www.upwork.com/jobs/~02${jobId}`
}

export const createHourlyRangeFromMinMaxBudget = (min: number, max: number) => {
  return `$${min.toFixed(2)} - $${max.toFixed(2)}`
}
export const createHourlyRangeFromFixedBudget = (min: number, max: number) => {
  return `$${min.toFixed(2)} - $${max.toFixed(2)}`
}

export const calculateHourlyRangeForUpworkAPI = (marketplaceJobPosting: IMarketplaceJobPosting): string | null => {
  if(marketplaceJobPosting.contractTerms.contractType === 'HOURLY' && marketplaceJobPosting.contractTerms.hourlyContractTerms){
    const { hourlyBudgetMin, hourlyBudgetMax} = marketplaceJobPosting.contractTerms.hourlyContractTerms
    return createHourlyRangeFromMinMaxBudget(hourlyBudgetMin, hourlyBudgetMax)
  } else if (marketplaceJobPosting.contractTerms.contractType === 'FIXED' && marketplaceJobPosting.contractTerms.fixedPriceContractTerms){
    const {amount: {displayValue}} = marketplaceJobPosting.contractTerms.fixedPriceContractTerms
    return `${displayValue} (Fixed)`
  }
}

export const getProjectLengthForUpworkAPI = (marketplaceJobPosting: IMarketplaceJobPosting): string | null => {
  if(marketplaceJobPosting.contractTerms.contractType === 'HOURLY' && marketplaceJobPosting.contractTerms.hourlyContractTerms){
    const {engagementDuration} = marketplaceJobPosting.contractTerms.hourlyContractTerms
    return engagementDuration?.label
  } else if (marketplaceJobPosting.contractTerms.contractType === 'FIXED' && marketplaceJobPosting.contractTerms.fixedPriceContractTerms){
    const {engagementDuration} = marketplaceJobPosting.contractTerms.fixedPriceContractTerms
    return engagementDuration?.label
  }
}

export const createUpworkProposalUrlFromProposalId = (id: string) => {
  return `https://www.upwork.com/nx/proposals/${id}`
}
...
export class PaymentRequiredException extends HttpException {
  constructor(message: string) {
    super(message, 402); // 402 status code
  }
}

export const generateInvoiceNumber = (counter: number) => {
  const prefix = "RT";
