...
import { createUpworkProposalUrlFromProposalId, formatMessages, getUpworkRestatFields, getUpworkRestatFieldsForManualBid } from "src/common/helpers";
...
import { IMessage, IVendorLead, IVendorProposal } from "src/types/bids";
...
export class BidService {
  ...
  public async createOrUpdateBidFromUpworkAPI( 
    {
      userId, contactId, jobId, bidProfileId, proposal, lead
    } : {
      userId: string, contactId: string, jobId: string, bidProfileId: string, proposal: IVendorProposal, lead: IVendorLead | null
    }
  )
  : Promise<{ message: string, bid: Bids }> {
    ...
    const leadDate = lead ? lead.createdAtDateTime : null
    ...
    if (bid) {
      await Bids.update({
        ...
        jobId,
        contactId,
        upworkProposalURL: createUpworkProposalUrlFromProposalId(proposal.id),
        coverLetter: proposal.proposalCoverLetter,
        bidResponse: !!leadDate,
        status: !!leadDate ? BidStatus.ACTIVE : BidStatus.PENDING,
        responseDate: leadDate,
        createdAt: proposal.auditDetails?.createdDateTime?.displayValue,
        isFromApi: true,
      }, 
      {
        where: {
