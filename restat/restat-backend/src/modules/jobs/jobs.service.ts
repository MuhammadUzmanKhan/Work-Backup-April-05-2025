...
import { calculateHourlyRangeForUpworkAPI, createJobUrlFromJobId, getProjectLengthForUpworkAPI } from "src/common/helpers";
...
export class JobService {
  ...
  public async getJobIdsByTags(tagIds: string[]) {
    let jobIds: string[] = [];
    const jobs = await Jobs.findAll({
      include: [
        {
  ...
  public async createOrUpdateJobFromUpworkAPI(workspaceId: string, marketplaceJobPosting: IMarketplaceJobPosting): Promise<{ message: string, job: Jobs }> {
    if (!marketplaceJobPosting.id) {
      throw new BadRequestException(jobsMessages.jobIdRequired);
    }

    const jobUrl = createJobUrlFromJobId(marketplaceJobPosting.id)
    ...
    if (job) {
      await Jobs.update({
        url: jobUrl,
        title: marketplaceJobPosting.content.title,
        description: marketplaceJobPosting.content.description,
        category: marketplaceJobPosting.classification?.category?.preferredLabel,
        experienceLevel: marketplaceJobPosting.contractTerms?.experienceLevel,
        hourlyRange: calculateHourlyRangeForUpworkAPI(marketplaceJobPosting),
        projectLength: getProjectLengthForUpworkAPI(marketplaceJobPosting),
      }, {
        where: {
          id: job.id
        }
    ...
    try {
      ...
      jobSkills.push(...marketplaceJobPosting.classification.skills?.map(skill => skill?.preferredLabel)) 
      jobSkills.push(...marketplaceJobPosting.classification.additionalSkills?.map(skill => skill?.preferredLabel)) 
      ...
      console.error(jobsMessages.jobCreatedError(error?.message), error);
      throw new InternalServerErrorException(error);
    }
  }
