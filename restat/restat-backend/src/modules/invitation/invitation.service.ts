...
export class InvitationService {
  ...
  public async sendInvitation(
    ...
    const associatedWithAnotherWorkspace: { name: string, email: string, role: string }[] = [];
    ...
    for (const element of members) {
      const trimmedEmail = element.email.trim().toLowerCase();
      const alreadyInvited = await Invitations.findOne({
        where: { email: trimmedEmail, companyId: user.companyId, status: INVITATION_STATUS.PENDING },
      });
      ...
      if (existingUser) {
        if (existingUser.companyId === user.companyId && !existingUser.deletedAt) {
          alreadyExistedMember.push({ ...element });
        } else if (existingUser.companyId !== user.companyId) {
          associatedWithAnotherWorkspace.push({ ...element });
        }
        else if (existingUser.companyId === user.companyId && existingUser.deletedAt) {
          newMember.push({
            name: element.name,
            email: trimmedEmail,
            role: element.role,
        ...
        ((user.role === ROLES.COMPANY_ADMIN || user.role === ROLES.OWNER) && element.role === ROLES.SUPER_ADMIN)
      ) {
        permissionRestrictedMemberAddition.push({ ...element });
      } else if (alreadyInvited) {
        resendInvitation.push({ ...element, id: alreadyInvited.id });
      } else {
        newMember.push({
          name: element.name,
          email: trimmedEmail,
    ...
    if (associatedWithAnotherWorkspace.length) {
      res.status(HttpStatus.CONFLICT).send({
        message: invitationsMessages.memberAlreadyExistInDifferentWorkspace,
        members: associatedWithAnotherWorkspace,
      });
      return;
    }

    if (permissionRestrictedMemberAddition.length) {
  ...
  public async acceptInvitation(id: string) {
    try {
      ...
      const isUserExist = await Users.findOne({ where: { email }, paranoid: false });

      if (isUserExist) {
        if (isUserExist.deletedAt) {
          // Restore the soft-deleted user
          await isUserExist.restore();
          // Update `createdAt` to reflect the new joining date
          await Users.update(
            {
          ...
          throw new UnprocessableEntityException(
            'User already exists! Please contact Restat Support for further assistance.',
          );
        }
      } else {
        // Create user record in the Users table if no existing user is found
