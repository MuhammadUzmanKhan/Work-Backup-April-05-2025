...
import { AssociatedId, WorkspaceDateFilterType } from "src/types/enum";
...
export class WorkspaceService implements OnModuleInit {
  ...
  public async createCompany(
    ...
    if (createWorkspaceDto.phoneNumber) {
      user.phoneNumber = createWorkspaceDto.phoneNumber
    };
    user.companyId = company.id;
    user.save();

    console.log("🚀 ~ WorkspaceService ~ user:", user)
  ...
  public async getCompanyById(id: string) {
    return await Workspaces.findByPk(id, {
      attributes: ['id', 'name', 'websiteUrl', 'logoUrl', 'companySize', 'location']
    });
  }
  ...
  public async getAllWorkspaces({
    ...
    const searchCondition = search
      ? {
        [Op.or]: [
          ...
          Sequelize.where(
            Sequelize.literal(
              `(SELECT "name" FROM "payment_plans" WHERE "payment_plans"."id" = "subscription"."planId")`
            ),
            { [Op.iLike]: `%${search}%` }
          ),
        ],
    ...
    let dateFilterCondition: any = {};

    if (dateFilterType && startDate && endDate) {
      if (dateFilterType === WorkspaceDateFilterType.LAST_ACTIVITY) {
        dateFilterCondition = {
          [Op.and]: [
            Sequelize.literal(
              `(SELECT MAX("lastActivityDate") FROM "users" WHERE "companyId" = "Workspaces"."id") BETWEEN '${startDate}' AND '${endDate}'`
            ),
          ],
        };
      } else if (dateFilterType === WorkspaceDateFilterType.CREATED_DATE) {
        dateFilterCondition = {
          createdAt: {
    ...
    // Query workspaces with pagination and filters
    const workspaces = await Workspaces.findAndCountAll({
      include: [
        {
          model: Subscriptions,
          attributes: ['nextBillingDate', 'isActive', 'billingCycle', 'planId'],
          include: [
            {
              model: PaymentPlans,
              attributes: ['name'],
      ...
      attributes: [
        ...
        [
          Sequelize.literal(
            `( SELECT count(*) FROM "users" where "companyId" = "Workspaces"."id" and "deletedAt" is null )`
          ),
          "teamCount",
        ],
        [
          Sequelize.literal(
            `( SELECT name FROM "users" where id = "Workspaces"."ownerId" )`
          ),
          "ownerName",
        ],
        [
          Sequelize.literal(
            `( SELECT email FROM "users" where id = "Workspaces"."ownerId" )`
          ),
          "ownerEmail",
        ],
        [
          Sequelize.literal(
            `( SELECT MAX("lastActivityDate") FROM "users" where "companyId" = "Workspaces"."id" )`
          ),
          "lastActivityDate",
        ],
        [
    ...
    const { rows: data, count: total } = workspaces;
    ...
    try {
      ...
      try {
        ...
        if (error instanceof NotFoundException) {
          // No subscription found, proceed without cancellation
          console.warn("No subscription found for workspace. Skipping cancellation.");
        } else {
  ...
  private async deleteWorkspaceAssociations(workspaceId: string) {
    try {
      ...
      await this.deleteStipeSubscriptionsAssociations(workspaceId);
  ...
  private async deleteUserAssociations(workspaceId: string,) {
    ...
    const userModels: any[] = [
      ...
      LinkedinReferences, OtpVerification, Sessions, Settings, UsersProfile, UserTargetHistory, Errors,
    ];
  ...
  private async deleteStipeSubscriptionsAssociations(workspaceId: string) {
    ...
    if (subscriptionIds.length === 0) {
      console.info(`No Stripe subscriptions found for companyId ${workspaceId}.`);
      return;
    }

    await Promise.all([
      SubscriptionDetails.destroy({
    ...
    console.info(`Stripe subscription associations deleted for workspaceId ${workspaceId}.`);
  }
  ...
  private async deleteDirectWorkspaceAssociations(workspaceId: string) {
    const workspaceIdModel: any[] = [
      ...
      WorkspaceDeletion, Integrations, Companies, Subscriptions, Invoices,
    ];

    const companyIdModels: any[] = [
      Configurations, ContactExperience, Invitations,
      Portfolios, Profiles, Settings, Users,
    ];
