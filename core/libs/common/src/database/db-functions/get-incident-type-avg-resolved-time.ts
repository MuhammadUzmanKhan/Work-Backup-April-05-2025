export const getIncidentTypeResolvedTime = `
CREATE OR REPLACE FUNCTION get_incident_types_avg_resolved_time(
    eventId INT,
    incident_type_ids INT[]
) RETURNS JSONB AS $$
DECLARE
    incident_id INT;
    open_time TIMESTAMP;
...
BEGIN
    ...
    FOR i IN 1..array_length(incident_type_ids, 1)
    LOOP
        ...
        FOR incident_id IN
            ...
            WHERE event_id = eventId
              AND incidents.incident_type_id = incident_type_ids[i]
        LOOP
        ...
        IF count_incidents_with_resolved > 0 THEN
            ...
            result := jsonb_set(result, ARRAY[incident_type_ids[i]::TEXT], jsonb_build_object('avg_resolved_time', avg_hours || ':' || avg_minutes || ''));
        ELSE
            result := jsonb_set(result, ARRAY[incident_type_ids[i]::TEXT], jsonb_build_object('avg_resolved_time', '00:00' || ''));
        END IF;
    END LOOP;
