export const getIncidentZoneResolvedTime = `
CREATE OR REPLACE FUNCTION get_incident_zone_resolved_time(
    eventId INT,
    incident_zone_ids INT[]
) RETURNS JSONB AS $$
DECLARE
    incident_id INT;
    open_time TIMESTAMP;
...
BEGIN
    ...
    FOR i IN 1..array_length(incident_zone_ids, 1)
    LOOP
        ...
        FOR incident_id IN
            ...
            WHERE event_id = eventId
              AND incidents.incident_zone_id = incident_zone_ids[i]
        LOOP
            ...
            INTO last_status, resolved_time
            FROM status_changes
            WHERE status_changeable_type = 'Incident'
              AND status_changeable_id = incident_id
        ...
        IF count_incidents_with_resolved > 0 THEN
            ...
            result := jsonb_set(result, ARRAY[incident_zone_ids[i]::TEXT], 
                              jsonb_build_object('avg_resolved_time', avg_hours || ':' || avg_minutes));
        ELSE
            result := jsonb_set(result, ARRAY[incident_zone_ids[i]::TEXT], 
                              jsonb_build_object('avg_resolved_time', '00:00'));
        END IF;
    END LOOP;
