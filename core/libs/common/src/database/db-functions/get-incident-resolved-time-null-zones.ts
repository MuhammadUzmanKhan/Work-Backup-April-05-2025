export const getIncidentResolvedTimeNullZones = `
CREATE OR REPLACE FUNCTION get_incident_resolved_time_null_zones(
    eventId INT
) RETURNS JSONB AS $$
DECLARE
    incident_id INT;
...
BEGIN
    result := '{}';
    incident_count := 0; -- Initialize incident count
    ...
    FOR incident_id IN
        ...
        WHERE event_id = eventId
            AND incidents.incident_zone_id IS NULL
    LOOP
        ...
        incident_count := incident_count + 1; -- Increment incident count for each iteration
    ...
    IF count_incidents_with_resolved > 0 THEN
        ...
        result := jsonb_set(result, ARRAY['Field Location Logged'], jsonb_build_object('incident_count', incident_count,'avg_resolved_time', avg_hours || ':' || avg_minutes || ''));
    ELSE
        result := jsonb_set(result, ARRAY['Field Location Logged'], jsonb_build_object('incident_count', incident_count,'avg_resolved_time', '00:00' || ''));
    END IF;
