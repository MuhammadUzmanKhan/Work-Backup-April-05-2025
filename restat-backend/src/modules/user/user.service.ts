...
export class UserService {
  ...
  public async getCompanyUsers(companyId: string, page: number, perPage: number = 20) {
    try {
      ...
      const companyOwner = await Workspaces.findOne({ where: { id: companyId }, attributes: ['ownerId', "name"] });
      ...
      const usersWithFlag = companyUsers.map(user => ({
        ...user.get({ plain: true }),
        companyOwner: user.id === companyOwner?.ownerId
      }));
  ...
  public async deleteUser(userId: string) {
    ...
    const companyOwner = await Workspaces.findOne({ where: { ownerId: userId }, attributes: ['ownerId'] });

    if (companyOwner?.ownerId === userId) throw new ConflictException(companiesMessages.companyOwnerCannotBeDeleted);
  ...
  public async updateUserRole(updateUserRoleDto: UpdateUserRoleDto) {
    ...
    const companyOwner = await Workspaces.findOne({ where: { ownerId: updateUserRoleDto.id }, attributes: ['ownerId'] });

    if (companyOwner?.ownerId === updateUserRoleDto.id) throw new ConflictException(EXCEPTIONS.COMPANY_OWNER_CANNOT_BE_UPDATED);
  ...
  public async updateStatus(id: string, updateStatusDto: UpdateStatusDto) {
    ...
    const companyOwner = await Workspaces.findOne({ where: { ownerId: id }, attributes: ['ownerId'] });

    if (companyOwner?.ownerId === id) throw new ConflictException(EXCEPTIONS.COMPANY_OWNER_CANNOT_BE_UPDATED);
  ...
  public async updateUser(updateUserDto: UpdateUserDto) {
    ...
    if (user.company?.ownerId === updateUserDto.id) throw new ConflictException(companiesMessages.companyOwnerCannotBeDeleted);
      ...
      await Users.update(
        {
          ...
          createdAt: updateUserDto.joiningDate,
