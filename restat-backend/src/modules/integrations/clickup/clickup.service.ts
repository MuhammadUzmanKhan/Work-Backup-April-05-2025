...
export class IntegrationsServiceClickup {
    ...
    public async getClickUpIntegrations(companyId: string) {
        try {
            return await Integrations.findOne({
                ...
                attributes: ['workspaceName', 'spaceName', 'folderName', 'listName', 'status', 'isFolderlessList', 'isSharedHierarchy', 'updatedAt', 'customFields'],
                include: [{
                    model: Users,
    ...
    public async getClickUpFields(companyId: string, listId: string): Promise<any> {
        try {
            ...
            const integratedData = await this.getClickUpIntegrations(companyId)
            if (integratedData.listId === listId) {
    ...
    async createOrUpdateAccessToken(user: Users, access_token: string): Promise<void> {
        try {
            ...
            if (existingIntegration) {
                await existingIntegration.update({ access_token, userId: user.id });
            } else {
    ...
    public async createClickUpTask({ priority = 3, assignees, notify_all, tags, ...taskProps }: CreateFieldProps, companyId: string) {
        try {
    ...
    public async updateClickUpTask(taskProps: CreateFieldProps, companyId: string, taskId: string) {
        try {
            const clickUp = await Integrations.findOne({
                where: {
                    type: INTEGRATION_TYPES.CLICKUP,
            ...
            const listId = clickUp?.listId;
            if (!listId) {
                return { status: false, message: clickUpMessages.clickupListNotFound, data: {} }
            }
            const query = new URLSearchParams({
                custom_task_ids: 'true',
                team_id: clickUp.workspaceId
